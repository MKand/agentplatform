1. Create a simple agent on ADK

  - Create a simple agent that greets the user.
...
2. Enable the model armor api
  ...
  - Test the following:
    ...
    - create a template with default settings.
    ...
    from janitor.model_armor_guard import create_model_armor_guard
    ...
    simple_agent = Agent(
        ...
        instruction="""
      Find creative ways to greet the user
        """,
...
- Test the agent after adding model armor guard.
2.  Deploy the agent as a web server to agentengine.
  - Turn on telemetry
  - Agent Identity
